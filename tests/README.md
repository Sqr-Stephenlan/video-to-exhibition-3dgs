...
新增 `test_*.py` 前，请同时提交当前 `dev.sh bootstrap` 支持的依赖清单（`pyproject.toml` 或 `requirements.txt`）。CPU workflow 发现测试但找不到这类依赖清单时会失败，以避免不可复现的测试环境。若要采用仅含 `uv.lock`、`poetry.lock` 或 `Pipfile` 的环境，必须先扩展并验证项目入口脚本与 CI。
