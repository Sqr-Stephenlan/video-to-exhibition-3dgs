...
3. 第三方仓库源码保留在 `third_party/`，如需改动第三方项目，在对应子仓库内部处理，并在主仓库文档中记录必要 patch 或运行参数。
4. 大体量视频、帧、深度图、模型、日志和训练输出不纳入 Git 跟踪，按 `.gitignore` 放入 `data/`、`outputs/` 或本地外部存储。
5. 实验记录包含输入视频、抽帧数量、有效关键帧、训练命令、输出模型、失败区域和质量指标。
...
这条线用于形成可展示结果，覆盖单展品、单展区和纹理较丰富的测试视频。
...
这条线对应课题研究重点：手机或 AR 眼镜视频输入、未知相机位姿、长视频、室内弱纹理和完整展厅重建。

## 五、阶段顺序

1. `docs/pipeline-design`：统一文档、目录约定和任务边界。
2. `feature/preprocess-video`：形成可复用的分段、抽帧和关键帧清单。
3. `feature/baseline-colmap-3dgs`：建立保底演示线。
...
8. `docs/evaluation-reporting`：贯穿记录实验，并集中整理验收材料。
...
| 分支 | 所属领域 | 相关能力 |
|---|---|---|
| `feature/preprocess-video` | 视频预处理 | 视频转码、抽帧、分段、图像质量检测 |
| `feature/baseline-colmap-3dgs` | 传统三维重建与 3DGS 基线 | SfM、COLMAP、官方 3DGS、gsplat |
| `research/longsplat-route` | 无位姿长视频 3DGS | LongSplat、MASt3R、CUDA / PyTorch 环境 |
| `research/depth-prior` | 视频深度估计与弱纹理辅助 | Video Depth Anything、深度图、置信度过滤 |
| `feature/segment-fusion` | 几何配准与片段融合 | Sim(3)、跨片段匹配、点云或 splat 变换 |
| `feature/export-viewer-annotation` | 模型导出、压缩与交互展示 | SuperSplat、splat-transform、PlayCanvas、JSON 标注 |
| `docs/evaluation-reporting` | 实验记录与验收材料 | 指标统计、对比评估、问题分析、汇报文档 |

## 九、风险与边界

1. LongSplat、MASt3R、Video Depth Anything 都依赖较复杂的 GPU / CUDA / PyTorch 环境，通常从短视频 smoke test 开始验证。
2. 单目深度不能直接等同于真实几何，只能作为深度先验或训练正则。
3. 分段融合是主要工程风险，涉及重叠帧保留、Sim(3) 变换记录、人工锚点或局部重采集。
4. 3DGS 的 `.ply` 保存的是 Gaussian 参数，不等同于传统 OBJ/mesh；如果需要碰撞或导航几何，需要额外生成辅助 mesh 或 voxel。
5. 展示端格式包括 `.sog`、`.compressed.ply` 或其他 Web 友好格式；超大原始 PLY 通常作为中间资产保存。
